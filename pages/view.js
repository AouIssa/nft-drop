...
async function updateRedeem(cid){
  fetch(`http://localhost:4000/updateDetailsFromCid/${cid}`).then(response => {
    return response.json()
  }).then((fef => {
...
export default function NFTViewer({ signer, provider, name, customContract
  ...
  useEffect(() => {
    ...
    const getNFTDetails = () => {
      fetch("http://localhost:4000/getNFTDetails").then(response => {
  ...
  const handleData = async () => {
    ...
    for (const a in data) {
      ...
      list.push(
        ...
        >
          ...
          <div style={{ fontSize: 15, textAlign: 'left', opacity: 0.77 }}>{data[a].description}</div>
          <div style={{ fontSize: 12, textAlign: 'left', opacity: 0.55 }}>{data[a].minPrice + " MATIC"}</div>
        </Card>
      );
    ...
    return (
      <StackGrid 
        columnWidth={150} gutterWidth={16} gutterHeight={16}
      >
        {galleryList}
      </StackGrid>
